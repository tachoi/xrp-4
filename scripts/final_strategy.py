#!/usr/bin/env python
"""Final optimized strategy - focus on letting winners run.

Key changes from previous:
1. No regime change exits (let trades reach TP or SL)
2. Later trailing stop activation
3. Better entry filters

Usage:
    python scripts/final_strategy.py
"""
...
def run_final_backtest(
    ...
    """Run final backtest - NO regime change exits."""
    ...
    for idx in range(3, len(df_bt)):
        ...
        # Position management - SL/TP only (no regime change exits)
        ...
        # Entry signals
        if position is None and conf >= min_confidence and equity > 0:
            signal = None
            ...
            # TREND_UP
            if regime == "TREND_UP":
                momentum_ok = ewm_ret_15m >= min_ewm_ret_15m
                h1_aligned = (not require_1h_alignment) or (price_above_1h_ema and ewm_ret_1h > 0)
            ...
            # TREND_DOWN
                ...
                momentum_ok = ewm_ret_15m <= -min_ewm_ret_15m
                h1_aligned = (not require_1h_alignment) or (price_below_1h_ema and ewm_ret_1h < 0)
            ...
            if signal is not None:
                ...
                max_size = equity / entry_price * 0.3
    ...
    if len(trades) > 0:
        ...
        results["exit_breakdown"] = exit_breakdown
    else:
        results["win_rate"] = 0
...
def main():
    ...
    parser.add_argument("--output_dir", type=Path, default=Path("outputs/final"))

    args = parser.parse_args()

    logger.info("=" * 70)
    logger.info("FINAL STRATEGY - NO REGIME CHANGE EXITS")
    logger.info("=" * 70)
    ...
    param_sets = [
        ...
        {"min_confidence": 0.6, "require_1h_alignment": True, "min_ewm_ret_15m": 0.0005,
         "sl_atr_mult": 1.5, "tp_atr_mult": 4.0, "use_trailing_stop": False},

        # Higher RR
        {"min_confidence": 0.6, "require_1h_alignment": True, "min_ewm_ret_15m": 0.0005,
         "sl_atr_mult": 1.0, "tp_atr_mult": 5.0, "use_trailing_stop": False},

        # With trailing stop
        {"min_confidence": 0.6, "require_1h_alignment": True, "min_ewm_ret_15m": 0.0005,
         "sl_atr_mult": 1.5, "tp_atr_mult": 5.0, "use_trailing_stop": True,
         "trail_activation_r": 2.0, "trail_atr_mult": 1.0},
        ...
        # Looser entry
        {"min_confidence": 0.5, "require_1h_alignment": False, "min_ewm_ret_15m": 0.0003,
         "sl_atr_mult": 1.5, "tp_atr_mult": 4.0, "use_trailing_stop": False},

        # Very high RR
        {"min_confidence": 0.6, "require_1h_alignment": True, "min_ewm_ret_15m": 0.0005,
         "sl_atr_mult": 1.0, "tp_atr_mult": 6.0, "use_trailing_stop": False},

        # Balanced
        {"min_confidence": 0.6, "require_1h_alignment": True, "min_ewm_ret_15m": 0.0004,
         "sl_atr_mult": 1.2, "tp_atr_mult": 4.0, "use_trailing_stop": True,
         "trail_activation_r": 1.5, "trail_atr_mult": 0.8},
    ]
    ...
    for i, params in enumerate(param_sets, 1):
        results = run_final_backtest(val_df, hmm_model, val_features, **params)
        ...
        if n_trades > 0:
            logger.info(
                f"         "
                f"AvgWin={results.get('avg_win_pct', 0):.3f}%, "
                f"AvgLoss={results.get('avg_loss_pct', 0):.3f}%, "
                f"Expect={results.get('expectancy', 0):.4f}%"
            )
    ...
    logger.info(f"Expectancy: {best_val_results.get('expectancy', 0):.4f}%")
    logger.info(f"Avg Win: {best_val_results.get('avg_win_pct', 0):.3f}%")
    logger.info(f"Avg Loss: {best_val_results.get('avg_loss_pct', 0):.3f}%")
    logger.info(f"\nBest params: {best_val_params}")
    ...
    test_results = run_final_backtest(test_df, hmm_model, test_features, **best_val_params)
    ...
    if "exit_breakdown" in test_results:
        logger.info(f"\nExit breakdown:\n{test_results['exit_breakdown']}")
    ...
    summary = {
        ...
        "validation_results": {
            ...
            "expectancy": best_val_results.get("expectancy", 0),
            "avg_win_pct": best_val_results.get("avg_win_pct", 0),
            "avg_loss_pct": best_val_results.get("avg_loss_pct", 0),
        },
        "test_results": {
            ...
            "avg_win_pct": test_results.get("avg_win_pct", 0),
            "avg_loss_pct": test_results.get("avg_loss_pct", 0),
        },
    }

    with open(args.output_dir / "final_results.json", "w") as f:
