"""Fast HMM (3m timeframe) for regime detection.

Implements a 4-state Gaussian HMM for micro-regime classification
on 3-minute OHLCV data.
"""
...
class FastHMM:
    """Fast HMM model for 3-minute timeframe regime detection.

    Uses a 4-state Gaussian HMM to classify market regimes:
    - State 0: RANGE (low volatility, mean-reverting)
    - State 1: TREND_UP (positive momentum)
    - State 2: TREND_DOWN (negative momentum)
    - State 3: TRANSITION (high uncertainty / regime change)

    Attributes:
        n_states: Number of HMM states (default 4)
        timeframe: Timeframe identifier (always "3m")
    ...
    def train(
        ...
        if n_samples < self.n_states * 10:
            raise ValueError(
                f"Insufficient samples: {n_samples} < {self.n_states * 10} minimum"
            )
        ...
        logger.info(
            f"FastHMM trained: {n_samples} samples, {n_features} features, "
    ...
    def _map_states_to_labels(self, features: np.ndarray) -> None:
        ...
        Uses multiple metrics to assign meaningful labels:
        ...
        # Get state variances (trace of covariance for each state)
        ...
        # Find primary return feature (ret_3m or ret_15m preferred)
        ...
        # TREND_UP: positive drift + high persistence
        trend_up_score = state_returns * 100 + trans_norm

        # TREND_DOWN: negative drift + high persistence
        trend_down_score = -state_returns * 100 + trans_norm

        # Greedy assignment (avoid duplicate labels)
        assigned = set()
        self.state_labels = {}

        # 1. TRANSITION first (usually most distinct - high variance, low persistence)
        ...
        if len(remaining) >= 2:
            ...
            if positive_states and negative_states:
                ...
                logger.warning(
                    f"FastHMM: All remaining states have same sign returns! "
        ...
        if len(labels) != len(set(labels)):
            logger.warning(f"Duplicate labels detected: {self.state_labels}")

        logger.debug(f"State label mapping: {self.state_labels}")
    ...
    def _find_primary_return_feature(self) -> int:
        ...
        Prefers exact matches (ret_3m, ret_15m) over partial matches.
        """
        # Exact matches first
        for i, name in enumerate(self.feature_names):
            if name == "ret_3m" or name == "ret_15m":
                return i
    ...
    def predict(
        ...
        # Clip scaled features to prevent extreme values (handle distribution shift)
        # Values beyond ±4 std are clipped to prevent numerical issues
        ...
        # Calculate entropy
        state_entropy = entropy(state_probs + 1e-10)  # Add small value to avoid log(0)

        # Update state history and calculate transition rate
    ...
    def predict_sequence(
        ...
        """Predict regime for a sequence of feature vectors.
    ...
    def _calculate_transition_rate(self) -> float:
        ...
        Returns:
            Fraction of recent steps that involved a state change
        """
    ...
    def save_checkpoint(self, run_id: str) -> HMMCheckpoint:
        ...
        Returns:
            HMMCheckpoint containing all model parameters
        """
        if not self._is_trained or self.model is None:
    ...
    def load_checkpoint(self, checkpoint: HMMCheckpoint) -> None:
        ...
        Args:
            checkpoint: HMMCheckpoint containing model parameters
        ...
        # Recreate model and set parameters
        self.model = self._create_model()
        ...
        # Initialize scaling parameters (will need to be set from data)
        ...
        self._is_trained = True
        logger.info(f"Loaded FastHMM checkpoint: run_id={checkpoint.run_id}")

    def set_scaling_params(self, mean: np.ndarray, std: np.ndarray) -> None:
        """Set feature scaling parameters.

        Used after loading a checkpoint to set the scaling parameters
